-- Final Working AI Chat System Setup
-- Checks for ALL missing columns before adding constraints
...
-- 2. Add ALL missing columns to ai_api_keys (if needed)
DO $$
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'ai_api_keys' AND column_name = 'api_key_name'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'ai_api_keys' AND column_name = 'api_key_encrypted'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'ai_api_keys' AND column_name = 'is_active'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'ai_api_keys' AND column_name = 'created_at'
    ) THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        -- Only add constraint if provider column exists
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'ai_api_keys' AND column_name = 'provider'
        ) THEN
    ...
    IF NOT EXISTS (
        ...
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'ai_api_keys' AND column_name = 'user_id'
        ) AND EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'ai_api_keys' AND column_name = 'provider'
        ) AND EXISTS (
...
-- 4. Now safely drop existing policies (tables exist now)
DO $$
BEGIN
