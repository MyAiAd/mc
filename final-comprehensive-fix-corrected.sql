-- =====================================================
-- FINAL COMPREHENSIVE FIX - CORRECTED VERSION
...
-- Method 1: Update table comments with timestamp (CORRECTED SYNTAX)
DO $$ 
BEGIN
    EXECUTE 'COMMENT ON TABLE public.goaffpro_affiliates IS ''GoAffPro affiliates - UPDATED ' || now()::text || '''';
    EXECUTE 'COMMENT ON TABLE public.affiliate_system_users IS ''Affiliate system users - UPDATED ' || now()::text || '''';
    EXECUTE 'COMMENT ON TABLE public.data_import_logs IS ''Data import logs - UPDATED ' || now()::text || '''';
    RAISE NOTICE '✅ Updated table comments with timestamps';
END $$;
