...
-- CREATE FUNCTION auth.email() RETURNS text -- Skipped: Extension function
--     LANGUAGE sql STABLE
--     AS $$
--   select 
--   coalesce(
...
-- --
-- 
-- CREATE FUNCTION auth.jwt() RETURNS jsonb -- Skipped: Extension function
--     LANGUAGE sql STABLE
--     AS $$
--   select 
--     coalesce(
...
-- --
-- 
-- CREATE FUNCTION auth.role() RETURNS text -- Skipped: Extension function
--     LANGUAGE sql STABLE
--     AS $$
--   select 
--   coalesce(
...
-- --
-- 
-- CREATE FUNCTION auth.uid() RETURNS uuid -- Skipped: Extension function
--     LANGUAGE sql STABLE
--     AS $$
--   select 
--   coalesce(
...
-- --
-- 
-- CREATE FUNCTION extensions.grant_pg_cron_access() RETURNS event_trigger -- Skipped: Extension function
--     LANGUAGE plpgsql
--     AS $$
-- BEGIN
--   IF EXISTS (
...
-- --
-- 
-- CREATE FUNCTION extensions.grant_pg_net_access() RETURNS event_trigger -- Skipped: Extension function
--     LANGUAGE plpgsql
--     AS $$
-- BEGIN
--   IF EXISTS (
...
-- --
-- 
-- CREATE FUNCTION extensions.pgrst_ddl_watch() RETURNS event_trigger -- Skipped: Extension function
--     LANGUAGE plpgsql
--     AS $$
-- DECLARE
--   cmd record;
...
-- --
-- 
-- CREATE FUNCTION extensions.pgrst_drop_watch() RETURNS event_trigger -- Skipped: Extension function
--     LANGUAGE plpgsql
--     AS $$
-- DECLARE
--   obj record;
...
CREATE FUNCTION public.calculate_multi_level_commissions(p_order_source text, p_order_id text, p_customer_email text, p_customer_name text, p_order_total numeric, p_order_date timestamp with time zone, p_product_category text DEFAULT 'default'::text, p_product_name text DEFAULT 'Product'::text, p_product_id text DEFAULT NULL::text, p_purchasing_affiliate_email text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
-- DECLARE -- Removed orphaned statement
...
ALTER TABLE realtime.messages_2025_06_12 OWNER TO supabase_admin;
